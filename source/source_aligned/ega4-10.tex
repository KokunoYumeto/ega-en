...
As $Z\cap\overline{X_0}\subset\overline{Z\cap X_0}$, it is trivial that $b')$ implies $a')$.
...
Then $S$ also satisfies conditions $2^\circ$ and $3^\circ$ of \sref{IV.10.6.1}: indeed, every local ring $\sh{O}_{S,s}$ is of dimension $0$ or $1$, and hence universally catenary \sref{IV.7.2.9}; on the other hand, an irreducible component $S'$ of $S$ is either reduced to a point or of dimension $1$, and for every closed point $s\in S'$, $\sh{O}_{S',s}$ is necessarily of dimension $1$.
\end{env}
...
We conclude immediately from this that if $X'=S(X)$, $Y'=S(Y)$ are two prealgebraic spaces over $k$, every morphism $X'\to Y'$ of \emph{$k$-ringed spaces} is also a morphism of $k$-ringed spaces \emph{in local rings}. Moreover, with the preceding notation, if $A$ and $B$ are $k$-algebras of finite type, the corresponding homomorphism makes $B$ an $A$-algebra of finite type; hence every morphism $X'\to Y'$ of $k$-ringed spaces is, by \sref{IV.10.9.6}, of the form $S(f)$, where $f:X\to Y$ is a morphism of $k$-preschemes.
