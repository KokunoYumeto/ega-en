...
Condition~$d)$ for $\mathrm{A}$ therefore implies (with the notations of~$e)$) that the ring $\mathrm{C}$ satisfies~$a)$, hence also $c)$; but as $\mathrm{C}$ is integral and $\dim(\mathrm{C})\geqslant 2$, one can apply condition~$c)$ to $\mathrm{C}$ by taking $\mathrm{T}$ to be the closed point of $\mathrm{Y}=\Spec(\mathrm{C})$, which proves that $d)$ implies $e)$.
...
$\operatorname{codim}(\{y'\}\cap\overline{\{x'\}},\overline{\{x'\}})\geqslant 2$.
...
This result shows that, to prove the equivalence of $a)$, $b)$ and $c)$, it suffices to show that $c)$ implies $b)$, and even that for every integral quotient ring $\mathrm{B}$ of $\mathrm{A}$, with field of fractions $\mathrm{K}$, the integral closure $\mathrm{B}'$ of $\mathrm{B}$ is a $\mathrm{B}$-module of finite type. Now, condition $c)$ implies that there exists $f\neq0$ in $\mathrm{B}$ such that $\mathrm{B}_f$ is integrally closed, and thus condition~(i) of \sref{IV.6.13.6} has been verified for $\mathrm{B}$. Condition~(ii) of \sref{IV.6.13.6} is also verified for $\mathrm{B}$: for every maximal ideal $\mathfrak{q}$ of $\mathrm{B}$, $\mathrm{B}_\mathfrak{q}$ is a quotient of a local ring $\mathrm{A}_\mathfrak{p}$, where $\mathfrak{p}$ is maximal in $\mathrm{A}$; since $\mathrm{A}_\mathfrak{p}$ is a Japanese ring by hypothesis, so is $\mathrm{B}_\mathfrak{q}$. Moreover, for every prime ideal $\mathfrak{r}$ of $\mathrm{B}$, $\mathrm{B}_\mathfrak{r}$ is a ring of fractions of some $\mathrm{B}_\mathfrak{q}$, where $\mathfrak{q}$ is a maximal ideal of $\mathrm{B}$, and is therefore again a Japanese ring. This proves the assertion.
...
\textbf{[36]} S.~\textsc{Abhyankhar}, ``Local uniformization of algebraic
...
$\rho_{X/Z}(\mathscr F)$ ($X$ a prescheme, $Z$ a subset of $X$ stable under
