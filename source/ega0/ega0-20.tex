...
\label{0.20.1.7}
Let $A$ be a ring, $C$ an $A$-ring, $L$ an $A$-bimodule, $E$ a trivial $A$-extension of $C$ by $L$, $p:E\to C$ the augmentation.
...
(ii) The fact that $d_{B/A}$ is an $A$-derivation of $B$ follows from the definition \sref{0.20.4.6} and from \sref{0.20.1.1}.
  ...
  To prove the universal property, recall that $\operatorname{D\acute{e}r.\ cont}_A(B,L)$ identifies canonically with the set of continuous $A$-algebra homomorphisms $u:B\to D_B(L)$ such that the composite $B\xrightarrow{u}D_B(L)\xrightarrow{q}B$ is the identity (where $q(b,0)=b$ is the projection; cf.\ \sref{0.20.1.6} and \sref{0.20.3.2}).
...
\tag{20.5.12.2}
  \label{0.20.5.12.2}
  0\to\Der_A(C,L)\xrightarrow{v^0}\Der_A(B,L)\xrightarrow{\partial}\operatorname{Exalcom}_B(C,L)
...
\xymatrix{
  ...
  \Upsilon_{C/B/\Lambda}=\Upsilon_{C/B/A}=\mathfrak{R}/\mathfrak{R}^2 \ar[r]^-{\delta_{C/B/A}} \ar[d]_{\ell} & \Omega^1_{B/A}\otimes_B C=\Omega^1_{E/A}\otimes_E C \ar[d] \\
